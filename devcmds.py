...
def Prod(devKey):    # Ask device a question, just to provoke a response
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, cmdRsp)
...
def Identify(devKey, durationS):    # Duration in seconds to flash the device's LED.  Use duration=0 to stop.
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+IDENTIFY:"+nwkId+","+ep+",0,"+durationStr, "DFTREP"]) # Identify for selected time
...
def SwitchOn(devKey):
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+RONOFF:"+nwkId+","+ep+",0,1", "OK"]) # Assume FFD if it supports OnOff cluster
        if zcl.Cluster.LevelCtrl in inClstr and dimLevel != 100:  # Queue up a dimming command if available and we're at a different dimness
            myqueue.EnqueueCmd(devKey, ["AT+LCMVTOLEV:"+nwkId+","+ep+",0,1,FE,0001", "OK"]) # Ensure fully bright ready to be turned on
...
def SwitchOff(devKey):
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+RONOFF:"+nwkId+","+ep+",0,0", "OK"]) # Assume FFD if it supports OnOff cluster
...
def Dim(devKey, level):
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+LCMVTOLEV:"+nwkId+","+ep+",0,1,"+levelStr+",000A", "DFTREP"]) # Fade over 1 sec (in 10ths)
        if level == 0:
            SwitchOff(devKey)
...
def Hue(devKey, hueDegree):
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+CCMVTOHUE:"+nwkId+","+ep+",0,"+hueStr+",00,0001", "DFTREP"]) # Fade over 100ms (in sec/10)
...
def Sat(devKey, satPercentage):
    ...
    if protocol == "ZigbeeHA" and nwkId != None and ep != None:
        ...
        myqueue.EnqueueCmd(devKey, ["AT+CCMVTOSAT:"+nwkId+","+ep+",0,"+satStr+",0001", "DFTREP"]) # Fade over 100ms (in sec/10)
